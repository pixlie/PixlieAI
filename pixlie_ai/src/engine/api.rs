...
impl APINodeItem {
    pub fn from_node(node: &ArcedNodeItem) -> APINodeItem {
        let payload: APIPayload = match &node.payload {
            Payload::Text(text) => {
                if node.labels.contains(&NodeLabel::WebPage)
                    || node.labels.contains(&NodeLabel::RobotsTxt)
                {
                    APIPayload::Text("".to_string())
                } else {
                    APIPayload::Text(text.clone())
                }
            }
