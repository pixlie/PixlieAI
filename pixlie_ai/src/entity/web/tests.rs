#[cfg(test)]
#[test]
fn test_webpage_scraper_basic_extraction_rlhf_book() {
    use crate::engine::engine::get_test_engine;
    use crate::engine::ArcedNodeItem;
    use crate::engine::{CommonEdgeLabels, CommonNodeLabels, Payload};
    ...
    let webpage = WebPage(RLHF_BOOK_INTRO.to_string());
    ...
    let link_node_ids = test_engine.get_node_ids_with_label(&"Link".to_string());
    assert_eq!(link_node_ids.len(), 15);

    let domain_node_ids = test_engine.get_node_ids_with_label(&"Domain".to_string());
    ...
    let all_domain_nodes: Vec<ArcedNodeItem> = domain_node_ids
        ...
        .filter_map(|node_id| match test_engine.get_node_by_id(node_id) {
            Some(node) => match node.payload {
                Payload::Domain(_) => Some(node.clone()),
                _ => None,
            },
            None => None,
        })
    ...
    //     test_engine.get_node_ids_with_label(&CommonNodeLabels::BulletPoints.to_string());
    // bullet_points_node_ids.sort();
    // assert_eq!(bullet_points_node_ids.len(), 3);
    //
    // let bullet_points_node = test_engine
    //     .get_node_by_id(bullet_points_node_ids.first().unwrap())
    //     .unwrap();
    // assert_eq!(
    //     match bullet_points_node.payload {
    //         Payload::Tree(ref text) => text.as_slice(),
    //         _ => &[],
    //     },
    ...
    //         _ => &[],
    //     },
    //     ["Chapter Summaries", "Target Audience", "How to Use This Book", "About the Author"]
    // );
}
...
fn test_extract_nested_lists() {
    ...
    let test_engine = get_test_engine();
    let arced_test_engine = Arc::new(&test_engine);
    let document = Html::parse_document(UL_HTML);
    fn traverse(
        given_el: ElementRef,
        parent_node_id: Option<NodeId>,
        parent_node_label: Option<String>,
        arced_test_engine: Arc<&Engine>,
    ) {
        for element in given_el.child_elements() {
            match element.value().name() {
                "body" => {
                    if element.has_children() {
                        traverse(element, None, None, arced_test_engine.clone());
                    }
                }
                "ul" => {
                    if element.has_children() {
                        let node_id = arced_test_engine
                            .get_or_add_node(
                                Payload::Tree,
                                vec!["UnorderedList".to_string()],
                                true,
                                None,
                            )
                            .unwrap()
                        ...
                        traverse(
                            element,
                            Some(node_id),
                            Some("UnorderedList".to_string()),
                            arced_test_engine.clone(),
                        );
                    }
                }
                "li" => {
                    if parent_node_label == Some("UnorderedList".to_string()) {
                        if let Some(parent_node_id) = parent_node_id {
                            let node_id = arced_test_engine
                                .get_or_add_node(
                                    Payload::Text(
    ...
    traverse(document.root_element(), None, None, arced_test_engine);
    let mut all_nodes = test_engine.get_all_nodes();
    all_nodes.sort_by(|a, b| a.id.cmp(&b.id));
    assert_eq!(all_nodes.len(), 13);

    let root_ul = test_engine.get_node_by_id(&all_nodes[0].id).unwrap();
    assert_eq!(
        root_ul.labels,
        vec!["UnorderedList".to_string(), "Tree".to_string()]
    );

    let connected_node_ids = test_engine
        .get_node_ids_connected_with_label(&root_ul.id, &CommonEdgeLabels::ParentOf.to_string())
        .unwrap();
    assert_eq!(connected_node_ids.len(), 2);
    let connected_nodes = connected_node_ids
        .iter()
...
fn test_extraction_from_hn_homepage() {
    ...
    let webpage = WebPage(HN_HOMEPAGE.to_string());
    let webpage_node_id = test_engine
        .get_or_add_node(
            Payload::FileHTML(webpage.clone()),
            vec![
    ...
    let children_of_webpage = test_engine
        ...
        .unwrap();
    // assert_eq!(children_of_webpage.len(), 100);
}
