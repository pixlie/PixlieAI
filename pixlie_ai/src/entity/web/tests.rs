...
use crate::engine::{ArcedNodeItem, Engine, NodeId};
...
//         )
//         .unwrap();
//     assert_eq!(children_of_webpage.len(), 68);
//
...
//         test_engine.get_node_ids_with_label(&CommonNodeLabels::UnorderedPoints.to_string());
...
fn test_extraction_from_hn_homepage() {
    ...
    let webpage_node_id = test_engine
        .get_or_add_node(
            Payload::FileHTML(WebPage(HN_HOMEPAGE.to_string())),
            vec![
                CommonNodeLabels::Content.to_string(),
                CommonNodeLabels::WebPage.to_string(),
            ],
    ...
    assert_eq!(
        ...
        vec!["Title", "Partial", "Text"]
    );

    // Count the number of Link nodes
    let link_node_ids = test_engine.get_node_ids_with_label(&CommonNodeLabels::Link.to_string());
    assert_eq!(link_node_ids.len(), 221);
}
