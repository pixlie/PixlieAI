...
impl Node for Topic {
    ...
    fn process(
        ...
        let evaluated_node_ids = engine
            .get_node_ids_connected_with_label(node_id, &CommonEdgeLabels::EvaluatedFor.to_string())
            .unwrap_or_else(|_| vec![]);

        let content_node_ids =
            engine.get_node_ids_with_label(&CommonNodeLabels::Content.to_string());
