...
impl Objective {
        ...
        match data_from_previous_request {
            Some(external_data) => match external_data {
                ExternalData::Response(response) => {
                    ...
                    for feature in parsed_response.features_needed_to_accomplish_objective {
                        match feature {
                            Feature::Crawler(crawl) => {
                                for web_search_keywords in crawl.web_search_keywords_for_objective {
    ...
    fn request_llm(node: &NodeItem, engine: Arc<&Engine>) -> PiResult<()> {
        ...
        let ts_crawl = clean_ts_type(&CrawlSpecification::export_to_string()?);
